                                ...
                                Ice_ComputeHMAC computeHMAC )
{
    ...
    if( retStatus == ICE_RESULT_OK )
    {
        ...
        /* This field is required as an attribute dueing creation of STUN packet. */
                                   ...
                                   uint32_t * pSendStunMessageBufferLength )
{
    ...
    if( retStatus == ICE_RESULT_OK )
    {
        ...
        if( retStatus == ICE_RESULT_OK )
        {
            *ppSendStunMessageBuffer = pIceAgent->stunMessageBuffers[ pIceAgent->stunMessageBufferUsedCount ];
            *ppCandidate = pIceCandidate;
        }
    }
    return retStatus;
                                     ...
                                     IceCandidate_t * pRemoteCandidate )
{
    ...
    if( retStatus == ICE_RESULT_OK )
    {
        ...
        if( retStatus == ICE_RESULT_OK )
        {
            memset( pIceCandidatePair->pTransactionIdStore, 0x00, ICE_DEFAULT_MAX_STORED_TRANSACTION_ID_COUNT );
                                                              ...
                                                              uint8_t * pTransactionIdBuffer )
{
    ...
    if( retStatus == ICE_RESULT_OK )
    {
        ...
        if( retStatus == ICE_RESULT_OK )
        {
            memcpy( pIceCandidatePair->pTransactionIdStore, stunHeader.pTransactionId, STUN_HEADER_TRANSACTION_ID_LENGTH );
    ...
    if( ( retStatus == ICE_RESULT_OK ) && ( deserializePacketInfo.errorCode == 0 ) )
    {
        ...
        case STUN_MESSAGE_TYPE_BINDING_REQUEST:
        {
            ...
            if( foundCandidatePair )
            {
                ...
                else
                {
                    ...
                    if( ( pIceCandidatePair->connectivityChecks & 1 ) == 0 )
                    {
                        ...
                        retStatus = ICE_RESULT_SEND_TRIGGERED_CHECK;
                    }
                    else
                    {
                        /* Create a response for received request. */
                        ...
                        retStatus = ICE_RESULT_SEND_RESPONSE_FOR_REMOTE_REQUEST;
                    }
                }
            ...
            else
            {
                ...
                retStatus = ICE_RESULT_CANDIDATE_PAIR_NOT_FOUND;
            }
                                             ...
                                             pReceivedStunMessageBuffer + STUN_HEADER_TRANSACTION_ID_OFFSET ) )
            {
                if( &deserializePacketInfo.stunAttributeAddress != NULL )
                {
                    ...
                    else
                    {
                        ...
                        retStatus = ICE_RESULT_CANDIDATE_NOT_FOUND;
                    }
                }
            }
            else
